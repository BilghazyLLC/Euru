        ...
        .get().then(querySnapshot => {
            if (querySnapshot) {
                ...
                if (snapshot && snapshot.exists) {
                    ...
                    $('#business-image').attr('src', business.image ? business.image : './assets/img/bg5.jpg').css({
                        'width': '100%',
                        'height': '370px'
                    });
...
const approveProvider = () => {
    ...
    db.collection('euru_users').doc(providerKey).set(data).then(() => {
        db.collection('euru_pending_approvals').doc(providerKey).delete()
            .then(() => {
